...
class XMLSecurityDSig
{
  ...
  <ds:SignedInfo>
    ...
    private function canonicalizeData(DOMElement $node, $canonicalmethod, $arXPath = null, $prefixList = null)
    {
        $exclusive = false;
        $withComments = false;
        ...
        $data = $node->C14N($exclusive, $withComments, $arXPath, $prefixList);
            ...
            foreach ($arTransforms as $transform) {
                ...
                if (is_array($transform) &&
                    ...
                    if (!empty($transform['http://www.w3.org/TR/1999/REC-xpath-19991116']['namespaces'])) {
                        foreach ($transform['http://www.w3.org/TR/1999/REC-xpath-19991116']['namespaces'] as $prefix =>
                                 $namespace) {
    ...
    public function sign($objKey, $appendToNode = null)
    {
        ...
        if ($xpath = $this->getXPathObj()) {
            $query = "./secdsig:SignedInfo";
            //dd($this->sigNode->C14N(false, false, ['query' => $query]));
            $nodeset = $xpath->query($query, $this->sigNode);
            if ($sInfo = $nodeset->item(0)) {
                ...
                //hack begin
                $sInfo->setAttribute('xmlns:ns2', "http://www.w3.org/2000/09/xmldsig#");
    ...
    static function staticAdd509Cert(
        $parentRef,
        $cert,
        $isPEMFormat = true,
        $isURL = false,
            ...
            if ($issuerSerial) {
                if ($certData = openssl_x509_parse("-----BEGIN CERTIFICATE-----\n" . chunk_split($X509Cert, 64, "\n") .
                    ...
                    if ($issuerSerial && !empty($certData['issuer']) && !empty($certData['serialNumber'])) {
                        ...
                        $x509Node =
                            $baseDoc->createElementNS(XMLSecurityDSig::XMLDSIGNS, 'ds:X509IssuerName', $issuerName);
