...
class XMLSecEnc
{
   ...
   <xenc:CipherData>
    ...
    public function encryptNode($objKey, $replace = true)
    {
        ...
        $encMethod = $this->encdoc->documentElement->appendChild($this->encdoc->createElementNS(
            XMLSecEnc::XMLENCNS,
    ...
    public function encryptKey($srcKey, $rawKey, $append = true)
    {
        ...
        if ($append) {
            $keyInfo = $root->insertBefore($this->encdoc->createElementNS(
        ...
        if (!empty($srcKey->name)) {
            $keyInfo = $encKey->appendChild($this->encdoc->createElementNS(
                'http://www.w3.org/2000/09/xmldsig#',
                'dsig:KeyInfo'
            ));
            $keyInfo->appendChild($this->encdoc->createElementNS(
                'http://www.w3.org/2000/09/xmldsig#',
                'dsig:KeyName',
                $srcKey->name
            ));
        ...
        if (is_array($this->references) && count($this->references) > 0) {
            ...
            foreach ($this->references as $name => $reference) {
                ...
                $dataRef =
                    $refList->appendChild($this->encdoc->createElementNS(XMLSecEnc::XMLENCNS, 'xenc:DataReference'));
